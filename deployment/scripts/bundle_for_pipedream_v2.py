    ...
    def resolve_dependencies(self, file_path: str) -> Dict[str, Set[str]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                if node.module and (node.module.startswith('src.') or node.module.startswith('library.src.')):
...
'''
    ...
    def clean_imports(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if line.strip().startswith('from src.') or line.strip().startswith('from library.src.'):
                continue
            if line.strip().startswith('import src.') or line.strip().startswith('import library.src.'):
                continue
    ...
    def collect_constants(self, constants: Set[str]) -> List[str]:
        ...
        const_file = "../../library/src/config/constants.py"
        collected = []
    ...
    def collect_functions(self, functions: Set[str]) -> List[str]:
        ...
        utils_files = [
            ...
            "../../library/src/utils/notion_utils.py",
            ...
            for func_name in functions:
                if func_name not in self.embedded_items:
                    ...
                    if source:
                        ...
                        display_path = utils_file.replace('../../library/', '')
                        collected.append(f"# From {display_path}")
