...
def get_notion_tasks(token: str, database_id: str) -> list:
    """
...
def build_calendar_event(task: Dict[str, Any]) -> Dict[str, Any]:
    """
    Build calendar event from Notion task.
...
def handler(pd: "pipedream") -> Dict[str, Any]:
    ...
    try:
        ...
        if not tasks:
            return {
                "message": "No tasks found in the specified database",
                "events": []
            }
        ...
        created_events = []
        for task in tasks:
            event = build_calendar_event(task)
