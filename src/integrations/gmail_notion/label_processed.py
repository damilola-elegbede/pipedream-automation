...
def get_label_id(service_headers: Dict[str, str], label_name: str) -> Optional[str]:
    """
    ...
    try:
        ...
        for label in labels:
            if label.get('name', '').lower() == label_name.lower():
                ...
                return label_id
        logger.error(f"Error: Label '{label_name}' not found in user's labels.")
        return None
...
def handler(pd: "pipedream") -> Dict[str, Any]:
    ...
    try:
        previous_step_output = safe_get(pd.steps, [PREVIOUS_STEP_NAME, "$return_value"])
    except Exception:
        logger.error(f"Error: Could not find return value from step '{PREVIOUS_STEP_NAME}'. Ensure the step name is correct and it exported data.")
    ...
    if not isinstance(previous_step_output, dict) or "successful_mappings" not in previous_step_output:
        logger.error(f"Error: Expected a dictionary with 'successful_mappings' key from step '{PREVIOUS_STEP_NAME}', but received: {type(previous_step_output)}")
    ...
    if not mappings_to_process:
        logger.info("No successful mappings received from the previous step. Nothing to label.")
        return {"status": "No data received", "labeled_messages": 0}

    if not isinstance(mappings_to_process, list):
         logger.error(f"Error: Expected 'successful_mappings' to be a list, but received type {type(mappings_to_process)}.")
    ...
    for item in mappings_to_process:
        if isinstance(item, dict) and "gmail_message_id" in item:
            ...
            logger.warning(f"Warning: Skipping item in 'successful_mappings' list as it's not a dictionary or missing 'gmail_message_id': {item}")

    if not message_ids_to_label:
         logger.warning("No valid Gmail message IDs found in the 'successful_mappings' data.")
    ...
    errors = []
    logger.info(f"Starting to add label '{LABEL_NAME_TO_ADD}' (ID: {target_label_id}) to {len(message_ids_to_label)} message(s)...")
    ...
    for index, msg_id in enumerate(message_ids_to_label):
        ...
        try:
            ...
            logger.error(f"  HTTP Error modifying message ID {msg_id}: {status_code} - {error_message}")
            if status_code == 403:
                 logger.error("  Error 403 (Forbidden): Check if the Gmail connection has the 'gmail.modify' scope enabled.")
            if status_code == 400:
                 logger.error(f"  Error 400 (Bad Request): Double-check the request body and Label ID '{target_label_id}'. Is the message ID '{msg_id}' valid?")
            if status_code == 404:
                 logger.error(f"  Error 404 (Not Found): Message ID '{msg_id}' might be invalid or deleted.")
            ...
            logger.error(f"  Request Exception modifying message ID {msg_id}: {req_err}")
            errors.append({"index": index + 1, "gmail_message_id": msg_id, "error": f"Request failed: {req_err}"})
        except Exception as e:
            logger.error(f"  An unexpected error occurred modifying message ID {msg_id}: {e}")
    ...
    # --- 6. Return Summary ---
    logger.info("\n--- Labeling Complete ---")
    logger.info(f"Successfully labeled messages: {len(successfully_labeled_ids)}")
