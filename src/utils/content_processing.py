    ...
    try:
        for part in path_parts:
            if isinstance(part, str):
                ...
                if isinstance(current_level, list) and 0 <= part < len(current_level):
                    current_level = current_level[part]
                else:
                    raise IndexError(
        ...
        if content is None:
            print(f"Warning: Content from step '{step_name}' at path '{'.'.join(map(str, path_parts))}' is None.")
            return ""
        if not isinstance(content, str):
            print(f"Warning: Expected a string from step '{step_name}' at path '{'.'.join(map(str, path_parts))}', but got {type(content)}. Attempting to convert.")
            return str(content)
        if not content.strip():
            print(f"Warning: Content from step '{step_name}' at path '{'.'.join(map(str, path_parts))}' is empty or whitespace.")
            return ""
        return content
    except (KeyError, IndexError, TypeError) as e:
        print(f"Error accessing data from step '{step_name}'. {str(e)} in path '{'.'.join(map(str, path_parts))}'")
        return None
    except Exception as e:
        print(f"An unexpected error occurred accessing data from step '{step_name}' at path '{'.'.join(map(str, path_parts))}': {e}")
        return None
...
def demote_headings(html_content: str) -> str:
    ...
    # Order of replacement is important to avoid multi-step demotion of the same tag
    replacements = [
        (r'<(/?)h5\b(.*?)>', r'<\1h6\2>'),
        (r'<(/?)h4\b(.*?)>', r'<\1h5\2>'),
        (r'<(/?)h3\b(.*?)>', r'<\1h4\2>'),
